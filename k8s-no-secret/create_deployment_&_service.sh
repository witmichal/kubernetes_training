...
kubectl create -f ./deployment-without-secret.yaml
kubectl wait --for=jsonpath='{.status.conditions[?(@.type=="Available")].status}'=True -f ./deployment-without-secret.yaml
kubectl create -f ./service-without-secret.yaml
